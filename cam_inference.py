...
# ------------------------------------------------
# ⭐ NEW — CORRECT YOLO11 NCNN POSTPROCESSING ⭐
# ------------------------------------------------
def yolo11_postprocess(mat_out_np, orig_w, orig_h, scale, pad_x, pad_y):
    ...
    # Expect (84,8400)
    if mat_out_np.shape != (84, mat_out_np.shape[1]):
    ...
    for idx in indices.flatten():
        ...
        dets.append((int(x), int(y), int(x+w), int(y+h), float(scores[idx]), int(class_ids[idx])))
    ...
    try:
        while True:
            ...
            if (now - last_time) >= frame_interval:
                ...
                print("Model output shape:", None if mat_out_np is None else mat_out_np.shape)
                ...
                cv2.imshow("USB Camera - NCNN 2FPS", frame)
                if out_writer:
